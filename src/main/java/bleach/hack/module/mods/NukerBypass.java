...
public class NukerBypass extends Module {
	...
	public NukerBypass() {
		super("NukerBypass", KEY_UNBOUND, Category.WORLD, "Breaks blocks around you",
				...
				new SettingSlider("Cooldown: ", 0, 4, 0, 0), new SettingToggle("All Blocks", true),
				new SettingToggle("Flatten", false), new SettingToggle("Rotate", false),
				new SettingToggle("NoParticles", false), new SettingMode("Sort: ", "Normal", "Hardness"),
...
//		}
		...
		if (getSettings().get(6).asToggle().state)
			FabricReflect.writeField(mc.particleManager, Maps.newIdentityHashMap(), "field_3830", "particles");

		if (getSettings().get(7).asMode().mode == 1)
			blocks.sort((a, b) -> Float.compare(mc.world.getBlockState(a).getHardness(null, a),
					mc.world.getBlockState(b).getHardness(null, b)));
		...
		for (BlockPos pos : blocks) {
			if (!canSeeBlock(pos) || mc.world.getBlockState(pos).getBlock() == Blocks.AIR
					...
					|| mc.world.getBlockState(pos).getBlock() == Blocks.LAVA || WorldUtils.isFluid(pos))
				continue;
			...
			if (mc.player.getPos().distanceTo(vec) > range + 0.5)
				continue;
			...
			if (getSettings().get(5).asToggle().state) {
				...
				mc.player.networkHandler.sendPacket(
						new PlayerMoveC2SPacket.LookOnly(mc.player.yaw, mc.player.pitch, mc.player.isOnGround()));
			...
			broken++;
			if (getSettings().get(0).asMode().mode == 0 || (getSettings().get(0).asMode().mode == 1
					&& broken >= (int) getSettings().get(8).asSlider().getValue()))
				return;
		}
	}
	...
	public boolean canSeeBlock(BlockPos pos) {
		...
		float yaw = mc.player.yaw
				+ MathHelper.wrapDegrees((float) Math.toDegrees(Math.atan2(diffZ, diffX)) - 90 - mc.player.yaw);
		float pitch = mc.player.pitch
				+ MathHelper.wrapDegrees((float) -Math.toDegrees(Math.atan2(diffY, diffXZ)) - mc.player.pitch);
		...
		return mc.world
				.raycast(new RaycastContext(mc.player.getCameraPosVec(mc.getTickDelta()), rayVec,
	...
	public List<BlockPos> getCube() {
		...
		BlockPos playerPos = new BlockPos(Math.floor(mc.player.getX()), Math.floor(mc.player.getY()),
