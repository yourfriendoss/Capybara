...
public class Window {
	...
	public void render(MatrixStack matrix, int mX, int mY) {
		...
		for (WindowButton w : buttons) {
			...
			textRend.drawWithShadow(matrix, w.text, bx1 + (bx2 - bx1) / 2 - textRend.getWidth(w.text) / 2,
					by1 + (by2 - by1) / 2 - 4, -1);
		}
		...
		if (drawIcon) {
			...
			MinecraftClient.getInstance().getItemRenderer().renderGuiItemIcon(icon, (int) ((x1 + 3) * 1 / 0.55),
					(int) ((y1 + 3) * 1 / 0.55));
		...
		/* window title */
		int iconWidth = drawIcon ? (icon.getItem() == Items.ARMOR_STAND || icon.getItem() == Items.POTION ? 13 : 14)
				: 4;
	...
	protected void drawBar(MatrixStack matrix, int mX, int mY, TextRenderer textRend) {
		...
		fillGradient(matrix, x1 + 2, y1 + 2, x2 - 2, y1 + 12, (selected ? 0xff0000ff : 0xff606060),
				(selected ? 0xff4080ff : 0xffa0a0a0));
