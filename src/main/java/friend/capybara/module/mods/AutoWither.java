...
public class AutoWither extends Module {
	...
	public AutoWither() {
		super("AutoWither", KEY_UNBOUND, Category.PLAYER, "automatically air places anvil to surround",
				new SettingToggle("Center", false), new SettingToggle("Debug", false));
	}
	...
	public void onTick(EventTick event) {
		...
		if (ticksPassed == 1) {
			if (
			// mc.world.getBlockState(targetPos.add(1, 0,
			// 1)).getMaterial().toString().contains("AIR") &&
			// mc.world.getBlockState(targetPos.add(1, 0,
			// -1)).getMaterial().toString().contains("AIR") &&
			mc.world.getBlockState(targetPos.add(1, 0, 0)).getMaterial().isReplaceable()
				...
				mc.interactionManager.interactBlock(mc.player, mc.world, Hand.MAIN_HAND, new BlockHitResult(
						mc.player.getPos().add(2, 0, 0), Direction.UP, targetPos.add(1, 0, 0), false));
				mc.player.swingHand(Hand.MAIN_HAND);
				mc.interactionManager.interactBlock(mc.player, mc.world, Hand.MAIN_HAND, new BlockHitResult(
						mc.player.getPos().add(2, 1, 0), Direction.UP, targetPos.add(1, 1, 0), false));
				mc.player.swingHand(Hand.MAIN_HAND);
				mc.interactionManager.interactBlock(mc.player, mc.world, Hand.MAIN_HAND, new BlockHitResult(
						mc.player.getPos().add(2, 1, 1), Direction.UP, targetPos.add(1, 1, 1), false));
				mc.player.swingHand(Hand.MAIN_HAND);
				mc.interactionManager.interactBlock(mc.player, mc.world, Hand.MAIN_HAND, new BlockHitResult(
						mc.player.getPos().add(2, 1, -1), Direction.UP, targetPos.add(1, 1, -1), false));
				mc.player.swingHand(Hand.MAIN_HAND);
		...
		if (ticksPassed == 2) {
			if (mc.world.getBlockState(targetPos.add(2, 2, 0)).getMaterial().isReplaceable()
				...
				if (getSetting(0).asToggle().state) {
					...
					mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionOnly(centerPos.x, centerPos.y,
							centerPos.z, mc.player.isOnGround()));
				}
				...
				mc.interactionManager.interactBlock(mc.player, mc.world, Hand.MAIN_HAND, new BlockHitResult(
						mc.player.getPos().add(2, 1, 0), Direction.DOWN, targetPos.add(1, 2, 0), false));
				mc.player.swingHand(Hand.MAIN_HAND);
				mc.interactionManager.interactBlock(mc.player, mc.world, Hand.MAIN_HAND, new BlockHitResult(
						mc.player.getPos().add(2, 1, 1), Direction.DOWN, targetPos.add(1, 2, 1), false));
				mc.player.swingHand(Hand.MAIN_HAND);
				mc.interactionManager.interactBlock(mc.player, mc.world, Hand.MAIN_HAND, new BlockHitResult(
						mc.player.getPos().add(2, 1, -1), Direction.DOWN, targetPos.add(1, 2, -1), false));
				mc.player.swingHand(Hand.MAIN_HAND);
