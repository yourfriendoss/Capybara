...
public class LiquidRemover extends Module {
	...
	public LiquidRemover() {
		super("LiquidRemover", KEY_UNBOUND, Category.PLAYER, "Automatically places netherrack where lava is",
				new SettingSlider("Range", 0.1, 5, 4.5, 1), new SettingSlider("R: ", 0.0D, 255.0D, 255.0D, 0),
				new SettingSlider("G: ", 0.0D, 255.0D, 69.0D, 0), new SettingSlider("B: ", 0.0D, 255.0D, 0.0D, 0),
				new SettingToggle("Air Interact", true), new SettingSlider("Tick Delay: ", 0.0D, 40.0D, 10.0D, 0)
						.withDesc("Ticks per block place to avoid kick for packet spam"));
	}
		...
		for (int y = -Math.min(range, player.getY()); y < Math.min(range, 255 - player.getY()); ++y) {
			for (int x = -range; x < range; ++x) {
				for (int z = -range; z < range; ++z) {
					...
					if ((this.mc.world.getBlockState(pos).getBlock() == Blocks.LAVA
							&& this.mc.world.getBlockState(pos).getFluidState().getLevel() == 8
							&& this.mc.world.getBlockState(pos).getFluidState().isStill())
					...
					if (mc.player.age % this.getSettings().get(5).asSlider().getValue() == 0) {
						...
						if (this.getSettings().get(4).asToggle().state) {
							mc.interactionManager.interactBlock(mc.player, mc.world, Hand.MAIN_HAND,
									new BlockHitResult(Vec3d.of(p), Direction.DOWN, p, true));
						} else {
