...
public class UI extends Module {
	...
	public UI() {
		super("UI", KEY_UNBOUND, Category.RENDER, "Shows stuff onscreen.",
				...
				new SettingToggle("Armor", true).withDesc("Shows your current armor").withChildren( // 9
						new SettingMode("Damage", "Number", "Bar", "Both")
								.withDesc("How to show the armor durability")),
	...
	public void onDrawOverlay(EventDrawOverlay event) {
		...
		if (getSetting(10).asToggle().state) {
			infoList.add("\u00a7fTime [\u00a7e" + new SimpleDateFormat(
					"MMM dd HH:mm:ss" + (getSetting(10).asToggle().getChild(0).asToggle().state ? " zzz" : "")
							+ (getSetting(10).asToggle().getChild(1).asToggle().state ? " yyyy" : ""))
									.format(new Date())
					+ "\u00a7f]");
		}
		...
		if (getSetting(4).asToggle().state) {
			...
			infoList.add("\u00a7fXYZ [" + (nether ? "\u00a74" : "\u00a7b") + pos.getX() + " " + pos.getY() + " "
					+ pos.getZ() + " \u00a7f(" + (nether ? "\u00a7b" : "\u00a74") + pos2.getX() + " " + pos2.getY()
					+ " " + pos2.getZ() + "\u00a7f)]");
		}
		...
		if (getSetting(3).asToggle().state) {
			PlayerListEntry playerEntry = mc.player.networkHandler
		...
		if (getSetting(5).asToggle().state) {
			...
			infoList.add(
					"\u00a7fTPS [" + getColorString((int) tps, 18, 15, 12, 8, 4, false) + tps + suffix + "\u00a7f]");
		}
		...
		if (getSetting(6).asToggle().state) {
			...
			if (time - lastPacket > 500) {
				...
				mc.textRenderer.drawWithShadow(event.matrix, text,
		...
		if (getSetting(9).asToggle().state && !mc.player.isCreative() && !mc.player.isSpectator()) {
			...
			for (ItemStack is : mc.player.inventory.armor) {
				...
				if (getSetting(9).asToggle().getChild(0).asMode().mode != 1) {
					...
					mc.textRenderer.drawWithShadow(event.matrix, s, (x + 19 - mc.textRenderer.getWidth(s)) * 1.333f,
							(y + 9) * 1.333f, 0xffffff);
					...
					if (is.isDamageable()) {
						...
						try {
							durcolor = MathHelper.hsvToRgb(
									((float) (is.getMaxDamage() - is.getDamage()) / is.getMaxDamage()) / 3.0F, 1.0F,
									1.0F);
						} catch (Exception e) {
						}

						mc.textRenderer.drawWithShadow(event.matrix, dur,
								(x + 10 - mc.textRenderer.getWidth(dur) / 2) * 1.333f, (y - 3) * 1.333f, durcolor);
					}
		...
		if (getSetting(16).asToggle().state && DiscordRPCManager.user != null) {
			infoList.add("\u00a7fDiscord [" + DiscordRPCManager.user.username + "#"
					+ DiscordRPCManager.user.discriminator + "\u00a7f]");
		}
