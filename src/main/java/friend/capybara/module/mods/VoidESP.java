...
public class VoidESP extends Module {
	...
	public VoidESP() {
		super("VoidESP", KEY_UNBOUND, Category.RENDER, "ESP for holes in the bedrock",
				new SettingSlider("Range", 0, 50, 15, 0), new SettingSlider("R: ", 0.0D, 255.0D, 255.0D, 0),
				new SettingSlider("G: ", 0.0D, 255.0D, 0.0D, 0), new SettingSlider("B: ", 0.0D, 255.0D, 0.0D, 0),
	...
	public void onTick(EventTick event) {
		if (mc.player.age % (int) this.getSettings().get(4).asSlider().getValue() == 0 && this.isToggled()
				&& mc.world.getRegistryKey().getValue().getPath().equalsIgnoreCase("the_nether")) {
