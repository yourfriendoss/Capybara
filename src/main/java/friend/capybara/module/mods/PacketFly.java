...
public class PacketFly extends Module {
	...
	public PacketFly() {
		super("PacketFly", KEY_UNBOUND, Category.MOVEMENT, "Allows you to fly with packets.",
				new SettingMode("Mode", "Phase", "Packet"), new SettingSlider("HSpeed", 0.05, 2, 0.5, 2),
				new SettingSlider("VSpeed", 0.05, 2, 0.5, 2), new SettingSlider("Fall", 0, 40, 20, 0),
		...
		if (getSetting(0).asMode().mode == 0) {
			...
			if (mc.player.headYaw != mc.player.yaw) {
				mc.player.networkHandler.sendPacket(
						new PlayerMoveC2SPacket.LookOnly(mc.player.headYaw, mc.player.pitch, mc.player.isOnGround()));
				return;
			}
			...
			mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionOnly(mc.player.getX() + mX,
					mc.player.getY() + mY, mc.player.getZ() + mZ, false));
			mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionOnly(mc.player.getX() + mX,
					mc.player.getY() - 420.69, mc.player.getZ() + mZ, true));
