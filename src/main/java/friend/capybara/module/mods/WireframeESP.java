...
import friend.capybara.utils.OutlineRenderUtils;
...
public class WireframeESP extends Module {
	...
	public WireframeESP() {
		super("WireframeESP", KEY_UNBOUND, Category.RENDER,
				"Allows you to see entities though walls. (rlly scuffed dont use)",
				...
				new SettingToggle("Vehicles", false).withDesc("Show Vehicles").withChildren(
						...
						new SettingColor("Color", 0.6f, 0.6f, 0.6f, false)
								.withDesc("Outline color for vehicles (minecarts/boats)")),
	...
	public void onWorldEntityRender(EventWorldRenderEntity event) {
		...
		} else if ((event.entity instanceof BoatEntity || event.entity instanceof AbstractMinecartEntity)
	...
	private void handleEsp(int setting, EventWorldRenderEntity event, float r, float g, float b) {
		...
		if (getSetting(setting).asToggle().state) {
			drawOutline(event.entity, event.matrix, r, g, b,
					(float) getSetting(setting).asToggle().getChild(0).asSlider().getValue());
		}
	}
