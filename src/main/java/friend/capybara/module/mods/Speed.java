...
public class Speed extends Module {
	...
	public Speed() {
		super("Speed", KEY_UNBOUND, Category.MOVEMENT, "Allows you to go faster, what did you expect?",
				new SettingMode("Mode", "Bhop", "MiniHop", "OnGround", "Strafe", "StrafeHop"),
	...
	public void onTick(EventTick event) {
		...
		if (getSetting(0).asMode().mode == 2) {
			...
			if (mc.player.forwardSpeed != 0.0F && !mc.player.horizontalCollision) {
				if (mc.player.verticalCollision) {
					mc.player.setVelocity(mc.player.getVelocity().x * (0.85 + speeds), mc.player.getVelocity().y,
							mc.player.getVelocity().z * (0.85 + speeds));
					jumping = true;
					mc.player.jump();
					// 1.0379
				}
			...
			if (mc.player.isOnGround())
				...
				mc.player.setVelocity(mc.player.getVelocity().x * (0.9 + speeds), -1,
						mc.player.getVelocity().z * (0.9 + speeds));
			...
			if (mc.player.forwardSpeed > 0 && mc.player.isOnGround()) {
				mc.player.jump();
				mc.player.setVelocity(mc.player.getVelocity().x * (0.65 + speeds), 0.255556,
						mc.player.getVelocity().z * (0.65 + speeds));
		...
		if (getSetting(0).asMode().mode == 3 || getSetting(0).asMode().mode == 4) {
			if ((forward == 0.0D) && (strafe == 0.0D)) {
				...
				mc.player.setVelocity(
						(forward * speedstrafe * Math.cos(Math.toRadians(yaw + 90.0F))
								+ strafe * speedstrafe * Math.sin(Math.toRadians(yaw + 90.0F))),
						mc.player.getVelocity().y, forward * speedstrafe * Math.sin(Math.toRadians(yaw + 90.0F))
								- strafe * speedstrafe * Math.cos(Math.toRadians(yaw + 90.0F)));
